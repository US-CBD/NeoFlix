...
class DetailsFilmFrame(ctk.CTkFrame):
    ...
    def configure_image(self):
        photo = ImageTk.PhotoImage(file=self.film.get_path(), size=(10, 10))
        image_label = ctk.CTkLabel(self.image_frame, image=photo)
        image_label.grid(row=0, column=0)

    def configure_details(self):
        self.configure_row(self.details_frame, 0, data_text=self.film.title, is_title=True)
    ...
    def configure_row(self, parent, row, label_text, data_text, is_textbox=False, is_color=False, is_title=False):
        ...
        if not is_textbox:
            ...
            textbox = ctk.CTkTextbox(parent, text=data_text, fg_color=self.get_color(self.film.rating), corner_radius=6)
